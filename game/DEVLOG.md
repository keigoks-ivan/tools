...
> 最後更新：2026-09-24。啟動載入、視覺、戰鬥與資源管理更新見下方；歷史設計與已知限制保留供交接。
...
> **改引擎後須更新 boot.js 的 `main.js?v=xxx`，並更新 index.html 的 `boot.js?v=xxx`；改樣式須更新 `ui.css?v=xxx`（防快取）。**

## 2026-09-24 首關美術、戰鬥節奏與行動操作

- 首爾首關加入共用店面圖集、可重複的石板路面 Canvas 貼圖、較收斂的建築窗景配色與街區細節；重複店面使用圖集既有格位。主城美術已落在首關，其他三關沒有因此全面重製。
- 首關改為三段連續突圍，每段擊破 30 名敵人，共 90 名後進入原有隊長 Boss 戰。敵人總上限桌機與觸控裝置一致為 18 名（包含出場中的敵人）；一般雜兵上限 15，最多同時混入 2 名精英。突圍補援以 4 人同方向小隊進場，巡遊小隊為 3 人；Runner 從側向接近。
- 保留可快速清除的雜兵群，不提高雜兵 HP。每段清場回復 25 點無雙；敵將被擊倒時會短暫震退附近雜兵並回復無雙。精英／Runner 攻擊間隔較短，紅色擴張警示圈在攻擊命中時抵達實際傷害半徑；一般敵人的同時攻擊仍受 4 名上限約束。
- J 連段在傷害判定幀後可接 K 派生重擊，各角色依連段段數走不同的單發重擊、突進或終結重斬；地面 K 長按蓄力、移動中 K 點按衝刺。增加短輸入緩衝，旋轉螢幕時清除按鍵殘留；觸控重擊使用 pointer capture，直向進行中的戰鬥暫停並提示轉橫向。
- Maria 主角與敵人複製模型的相同骨架改為在每個複製角色內共用，避免分件網格持有重複 Skeleton。角色仍沿用既有 Maria 模型與配色／裝備，Mira、Zoey 專屬模型沒有完成。
- 敵人載入後，在改材質前合併相同骨架／材質／變換的身體部件，保留全部頂點、法線、UV、權重與動畫。60 FPS 限制改用固定時序，避免每次回呼的時間抖動累積為跳幀；不放開幀率上限，也不自動降低解析度。
- 敵人滿額時不提前切入空的頭目階段；等空位後再重試並顯示頭目對話與血條。
- Maria runtime GLB 的 base-color 圖片改用無損 WebP；Python 檢查會比對解碼後像素，並檢查模型幾何、骨架及動畫。未移除模型細節或動畫取樣。此變更沒有宣稱已達到特定幀率或手機實機效能。
- 驗證指令與目前測試範圍見 [tests/README.md](tests/README.md)。手機實機、四關完整通關與長時間效能測試仍未完成；靜音 Chrome 操作結果另記於 [QA-20260924.md](QA-20260924.md)。

## 2026-09-23 第二輪：精簡實際載入資料，保留畫質

- 新增 `scripts/prepare_runtime_assets.py`，從原始 GLB 產生 `assets/runtime/` 遊戲版本。原始素材保留，授權不變；產物不縮圖、不重新編碼圖片、不減面、不抽減動畫取樣。
- 主角原本帶有 5,627,744 bytes 的 normal map，但 toonify 只保留 baseColor map，這張貼圖不參與目前畫面。遊戲版本移除這個無效貼圖與引用；保留完整 2048×2048 原始色彩 PNG、兩組網格及 13 個動畫。
- 敵人只保留 E_ANIMS 中存在的動畫（含保守保留的 Taunt），重新整理未使用的 accessor / bufferView 與二進位片段。骷髏 95 → 21 個動畫，其他敵人 76 → 18 個動畫；保留的軌道、關鍵影格、骨架與所有幾何資料不變。
- 第一關三個 GLB 合計由 27,132,900 → 14,631,776 bytes（約少 46%）；這是檔案體積差異，不代表實際載入時間降低 46%。
- 主角與第一種敵人重疊下載；小型城市道具每批兩份下載，減少串行網路等待，仍避免一次解析全部素材。維持所有解析度、MSAA、陰影、反射、Bloom、模型、特效設定。
- Python 比對檢查原始與產物中的幾何、骨架、使用中的動畫取樣及色彩貼圖；Node 檢查按需載入和有限並行。依使用者要求未啟動 Chrome、遊戲或 GPU，實際效能仍待驗證。

## 2026-09-23 首頁與關卡延後載入

- HTML 改載入輕量 boot.js。開頁只顯示封面與選單；按開始後才動態匯入 main.js / Three.js，建立 WebGL 並下載模型。首頁靜音設定會交給引擎，重複開始操作不會重複初始化。
- 五個場景建立函式（首關城市及遠景、後三關）保留原本內容，改成進入關卡時呼叫；靜態合併也只處理本關。已建立關卡保留供重試，不會重複建立；未進入的關卡不先建置。
- 敵人依關卡載入：第一關 minion/warrior、第二關 minion/barbarian、第三關 minion/knight、第四關 minion/rogue/barbarian。模型依序載入並重用，首次少下載約 10.4 MiB 的三個後續關卡模型；這是檔案量差異，不是實測效能提升幅度。
- 保留抗鋸齒、解析度、燈光及模型細節。街景反射仍在按開始後準備第一關時產生一次，第四關地面建立後套用相同反射。主角實體延後到選角時建立，避免預先建立後立即替換。
- 換關顯示載入畫面，暫停更新與音訊、清空輸入；失敗時停留並可重試。首頁初始化失敗則由使用者按鈕重新整理，避免同頁重建 WebGL。
- 驗證僅使用語法檢查、靜態審閱及隔離依賴的 Node 測試；未開啟 Chrome、遊戲或 WebGL，也未測量真實載入時間。本次不宣稱已排除 Chrome 當機根因。

## 2026-09-14 視覺與資源管理更新

- 依 DMC5、Scarlet Nexus、Hades II 的方向重新整理主選單、選角、HUD 與結算介面；保留原有素材與四關玩法。設計來源、分工與範圍見 VISUAL-PLAN.md。
- 四關燈光配色、較窄且漸細的刀光與短促命中火花；降低泛白。密集敵群時鏡頭拉高拉遠並聚焦角色，奔跑與大絕視角平滑變化。
- 標題／選角不持續繪製 3D；死亡與勝利覆蓋層顯示後停止繪製。切至背景停止迴圈，清空鍵盤與觸控狀態，回來時重設時間避免跳幀。
- 靜態世界依材質與區域合併，排除動畫與遮擋控制物件；不減少模型頂點／三角形。特效數量有上限，避免極端連段無限累積。
- 退場敵人、拾取物、飛行彈與換關物件釋放所擁有的資源，保留共享貼圖及 Sprite 共用幾何。場景合併與資源清理各有獨立模組與測試。
- 開始前若已靜音，不建立 AudioContext、不下載／解碼音樂、不排程音效；已播放後靜音會暫停音訊。移除空白 looping audio workaround。音樂按需載入，過期非同步結果不會開始播放。
- 開場前同步角色實際位置；任務文字只在變化時更新。修正手機按鍵重疊、短螢幕布局、直向選角被旋轉提示遮住等問題。

### 驗證與限制

- Node 測試涵蓋靜音、解碼競態、換曲、背景／恢復、關卡重設、場景位置／法線／材質保留，以及共享圖形資料的清理保護。指令見 tests/README.md。
- 靜音瀏覽器實測完成選角、第一關開場、移動／跳躍、連段／重擊、武器切換及死亡結算；Rumi 9 次擊殺／最高 11 連段、Mira 40 次擊殺／最高 40 連段；Zoey 開場、攻擊與死亡後重試也通過。未觀察到先前逾時與當機，音樂請求數為零。
- 靜態介面驗證 844×390、568×320 橫向與 390×844 直向選角；手機實機、四關完整通關與長時間有聲壓力測試未涵蓋。
- 先前 Chrome 主程序當機的確切原因仍未證實；本次修正已知資源浪費，不宣稱已證明或徹底排除所有 Chrome 當機原因。
...
3. 三位角色已換成各自的幾何、髮型與服裝（2026-09-24）；仍共用既有動作骨架，尚無獨立臉部表情或即時髮絲物理。
...
- 新建 Rumi（紫色長辮、象牙短外套、長劍）、Mira（玫紅長髮、深色背心、重劍）、Zoey（雙丸子頭、青綠短外套、短刃）三份 GLB；替換舊 Maria 的身體、頭髮與武器幾何。保留來源骨架和全部 13 組動作，沒有重製動作捕捉。
- 每位四個合併蒙皮表面、頂點色、不含圖片貼圖，檔案約 1.65–1.69 MiB。生成只在離線製作時執行，遊戲不做現場建模。原始 Maria 資產保留供重建與回歸比對。
- 修正膝踝權重交界、握劍位置、肩部接縫、服裝與內層穿插。長辮與長髮下段混合頭／脊椎／髖部權重，減少轉身穿入背部；不是髮絲物理模擬。
- 啟動只先準備 Rumi；Mira／Zoey 選中才載入，同角色並行請求共用快取。失敗可重試，載入期間鎖定卡片並清除殘留輸入。
- 新造型跳過舊的疊加髮型／護甲／替代武器，保留三人的武器光與軌跡。每次建立角色獨立持有材質；更換時釋放自身骨架、特效與陰影，保留快取幾何。
- 新增 model-lab.html，可單獨檢查正／側／背／臉部、綁定姿勢與五個主要動作。一次載入一人、動畫預覽上限 30 FPS，暫停／背景時停繪；遊戲幀率設定不變。
- Node 全套 67 項通過。新增實際 GLB 的尺寸、無圖片、13 動作、權重、骨架、動畫變形／姿態界限檢查，以及載入競態、錯誤重試、巢狀模型識別與快取資源保護。實際畫面驗證與裝置限制見 QA-20260924.md。
