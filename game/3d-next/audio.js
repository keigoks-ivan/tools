...
// 0.25 s 無聲 WAV（8 kHz 單聲道），只給 iOS 舊版切換音訊類別用
...
export function createAudio({ baseUrl = '../assets/audio/march/', storage = globalThis.localStorage, fetchImpl = globalThis.fetch?.bind(globalThis),
  ...
  // iOS 靜音開關：Web Audio 預設跟著靜音。Safari 16.4+ 用 audioSession 改成 playback；
  // 舊版改在手勢內循環播放一段無聲 <audio>，讓系統把整頁歸類為媒體播放
  let sessionEl = null;
  function preferPlaybackSession() {
    ...
    try {
      if (!sessionEl && typeof globalThis.Audio === 'function') {
        sessionEl = new globalThis.Audio(SILENT_WAV);
        sessionEl.loop = true;
  ...
  const api = {
    ...
    unlock() {
      ...
      if (!settings.muted) preferPlaybackSession();
      if (!ctx) {
